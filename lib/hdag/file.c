                 ...
                 uint32_t edge_num)
{
    ...
    file.size = hdag_file_size(hash_len, node_num, edge_num);
    ...
    *(file.header = file.contents) = header;
    file.nodes = (struct hdag_file_node *)(file.header + 1);
    file.edges = (struct hdag_file_edge *)(
            (uint8_t *)file.nodes +
            hdag_file_node_size(file.header->hash_len) *
